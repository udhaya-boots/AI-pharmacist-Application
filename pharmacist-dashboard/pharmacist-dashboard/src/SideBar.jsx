            ...
            <div className="mt-auto pt-6 border-t border-indigo-700">
                <div className="flex items-center space-x-3 mb-4">
                    <div className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-sm font-bold">
                        {pharmacist?.name ? pharmacist.name.split(' ').map(n => n[0]).join('') : 'JP'}
                    </div>
                    <div className="flex-1">
                        <p className="text-sm font-medium">{pharmacist?.name || 'Dr. John Peter'}</p>
                        <p className="text-xs text-indigo-300">{pharmacist?.specialization || 'Clinical Pharmacy'}</p>
                    </div>
                </div>
                
                <button 
                    onClick={handleLogout}
                    className="flex items-center space-x-2 text-sm text-indigo-300 hover:text-white transition-colors duration-200 w-full"
                >
                    <LogOut size={16} />
                    <span>Logout</span>
                </button>
